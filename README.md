...
**Fase 4 — água e evolução básica de casas**: mapa isométrico 30×30, construção manual, modelo separado de estado da cidade e simulação simples por tick em TypeScript + Vite + PixiJS, sem React.
...
- Overlay opcional de água, desenhado sem novos sprites, mostra os tiles cobertos por poços em raio Manhattan 3.
- Sprites reais da Caesaria, alinhados pela base do tile e ordenados de trás para a frente; casas nível 2 recebem tint simples.
...
- Cada construção cria um `Building { id, type, x, y }`; casas também guardam `level`, `hasRoadAccess`, `hasWater` e `upgradeProgress`.
- O painel mostra dinheiro, ferramenta selecionada, tick, estatísticas de casas/água, toggle de overlay e feedback da última ação.
- Tiles ocupados, coordenadas fora do mapa e dinheiro insuficiente são rejeitados sem alterar cidade ou saldo.
- **Reset** recria `CityState`, restaurando tiles, `buildings[]`, `resources.money`, `simulation.tick` e os edifícios iniciais; mantém a ferramenta selecionada e o estado do overlay.

O mapa completo é redesenhado após construção válida, reset, tick de simulação, toggle de overlay ou redimensionamento. Não há pan/zoom, demolição, comida/workers, farms/granaries/markets, walkers/pathfinding, backend ou advisor/IA.
...
src/
  ...
  rendering/MapRenderer.ts   Camadas, profundidade, overlay de água, refresh e enquadramento
  rendering/GridMath.ts      Conversão isométrica nos dois sentidos
  simulation/CityState.ts    Estado, seed, buildings[], resources, simulação, custos e validação de construção
  simulation/Simulation.ts   Tick, cobertura de água, acesso a estrada, evolução e estatísticas de casas
  simulation/Tile.ts         Coordenadas, terreno e referência buildingId opcional
  ui/BuildPanel.ts           Painel HTML: ferramentas, custos, dinheiro, stats, overlay, feedback e reset
...
`gridToScreen` devolve o centro do losango em coordenadas locais do mapa, usando tiles de 120×60. `screenToGrid` recebe essas mesmas coordenadas locais, devolve o tile mais próximo e retorna `null` para valores não finitos. O input usa `map.toLocal(event.global)` antes da conversão; `placeBuilding` em `CityState` valida coordenadas inteiras e limites, ocupação por `buildingId` e saldo em `resources.money` antes de qualquer mutação. `INITIAL_MONEY` e `BUILD_COSTS` nesse ficheiro configuram a economia. `simulation.tick` começa em 0 e é incrementado por `simulateTick` a cada segundo. O renderer deriva terreno de `city.tiles`, edifícios de `city.buildings` e cobertura de água de poços; refresh destrói os objetos visuais antigos sem destruir as texturas partilhadas.
