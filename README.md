...
**Fase 8 — advisor mock determinístico**: mapa isométrico 30×30, construção manual, modelo separado de estado da cidade, simulação simples por tick, análise determinística de problemas urbanos e advisor mock que transforma issues em planos legíveis sem LLM, execução automática ou mutação da cidade.
...
`build` verifica TypeScript em modo estrito e gera `dist/`; `test` executa os testes unitários Vitest do analyzer e do advisor mock; `preview` serve esse build localmente. `node_modules/` e `dist/` estão ignorados pelo Git. **O build é apenas para validação local: inclui os assets temporários e não deve ser publicado.**
...
- O painel Advisor tem botão **Analyze city**; gera um `AdvisorPlan` determinístico com resumo do principal problema, raciocínio, ações mock, custo estimado, impactos esperados e riscos. **Approve** só reporta execução futura não implementada e não altera a cidade; **Reject** limpa o plano.
...
O mapa completo é redesenhado após construção válida, reset, tick de simulação, toggle de overlay ou redimensionamento. Workplaces inativos aparecem com alpha reduzido e tint vermelho/cinzento. Não há pan/zoom, demolição, walkers/pathfinding, commute, salários, impostos, migração, desirability, múltiplos tipos de comida, backend, LLM, validação/execução real de ações do advisor ou persistência de recomendações.
...
src/
  ...
  ui/AdvisorPanel.ts         Painel HTML do advisor: Analyze city, plano, Approve sem execução e Reject
