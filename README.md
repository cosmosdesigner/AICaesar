...
**Fase 5 — economia mínima de comida**: mapa isométrico 30×30, construção manual, modelo separado de estado da cidade e simulação simples por tick em TypeScript + Vite + PixiJS, sem React.
...
- Farms produzem 2 unidades de comida por tick, mas só acumulam quando há granary; cada granary adiciona 100 de capacidade.
- Markets cobrem casas em raio Manhattan 4; casas cobertas consomem 1 comida a cada 2 ticks enquanto houver stock.
- Casas com estrada, água e comida evoluem para nível 3 após 5 ticks de serviço alimentar; casas sem comida permanecem no nível atual.
- Overlays opcionais de água e comida, desenhados sem novos sprites, mostram cobertura de poços e markets; no overlay de comida, casas sem comida recebem tint laranja.
...
- Cada construção cria um `Building { id, type, x, y }`; casas também guardam `level`, `hasRoadAccess`, `hasWater`, `hasFood` e `upgradeProgress`; granaries recebem `storedFood` temporário, com o stock efetivo centralizado em `resources.food`.
- O painel mostra dinheiro, ferramenta selecionada, tick, estatísticas de casas/água/comida, toggles de overlay e feedback da última ação.
- Tiles ocupados, coordenadas fora do mapa e dinheiro insuficiente são rejeitados sem alterar cidade, saldo ou comida.
- **Reset** recria `CityState`, restaurando tiles, `buildings[]`, `resources.money`, `resources.food`, `simulation.tick` e os edifícios iniciais; mantém a ferramenta selecionada e os estados dos overlays.

O mapa completo é redesenhado após construção válida, reset, tick de simulação, toggle de overlay ou redimensionamento. Não há pan/zoom, demolição, walkers/pathfinding, workers, múltiplos tipos de comida, backend ou advisor/IA.

Verificação manual: construir Road num tile vazio (saldo 496), selecionar House e clicar no mesmo tile (erro, saldo 496), construir Farm, Granary e Market em tiles vazios. Sem Granary, Farms não acumulam comida; com Granary, comida cresce até a capacidade. Casas em raio de Market consomem comida nos ticks pares e podem chegar ao nível 3 se também tiverem estrada e água. Reset deve restaurar saldo 500, comida 0, tick 0 e a cidade seedada.
...
src/
  ...
  simulation/Simulation.ts   Tick, água, comida, acesso a estrada, evolução e estatísticas
  simulation/Tile.ts         Coordenadas, terreno e referência buildingId opcional
  ui/BuildPanel.ts           Painel HTML: ferramentas, custos, dinheiro, stats, overlays, feedback e reset
