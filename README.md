...
**Fase 6 — trabalhadores e emprego**: mapa isométrico 30×30, construção manual, modelo separado de estado da cidade e simulação simples por tick em TypeScript + Vite + PixiJS, sem React.
...
O mapa completo é redesenhado após construção válida, reset, tick de simulação, toggle de overlay ou redimensionamento. Workplaces inativos aparecem com alpha reduzido e tint vermelho/cinzento. Não há pan/zoom, demolição, walkers/pathfinding, commute, salários, impostos, migração, desirability, múltiplos tipos de comida, backend ou advisor/IA.
